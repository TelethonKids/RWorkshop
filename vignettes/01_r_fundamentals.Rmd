...
author: Matt Cooper, Paul Stevenson, Sebastian Rauschert
date: August 08, 2019
output:
...
- <code>as.factor()</code> does not accept additional arguments
    - see <code>levels</code> defaults.
- <code>relevel</code> lets you set the 'baseline' level
...
- broady the same as a <code>data.frame</code>, but less unruly (print)
...
- Entire strucutre is one class (all <code>numeric</code> or <code>character</code> etc)
...
- A single character in a column that isn't <code>0-9</code> or <code>.</code> will mean that column is read in as <code>character</code>
...
- Some packages are installed direct from the developer
    - Tnstalled via <code>devtools::install_github()</code>
...
- Running <code>.libPaths()</code> will show where (on your computer) your packages are stored
- Running <code>citation("ggplot2")</code> will tell you how to cite the package
- Beware of clashes
